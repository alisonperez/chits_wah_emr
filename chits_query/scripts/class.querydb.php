...
//		print_r($bilang);
	...
	function process_philhealth_list($quesno){
		...
		echo "window.alert('When querying for patients with PhilHealth, the EXPIRATION DATE must be inclusive of the START and END DATE you are setting. You may also query PhilHealth patient per barangay')";
		echo "</script>";
		...
		if($_POST["sel_brgy"]!='all'):		
			//$q_philhealth = mysql_query("SELECT a.patient_id FROM m_patient_philhealth a, m_family_address b, m_family_members c,m_lib_barangay d, m_patient e WHERE a.expiry_date BETWEEN '$sdate' AND '$edate' AND a.patient_id=.patient_id AND c.family_id=b.family_id AND b.barangay_id=d.barangay_id AND a.patient_id=e.patient_id AND b.barangay_id='$_POST[sel_brgy]' ORDER by d.barangay_name ASC, e.patient_lastname ASC,a.expiry_date ASC") or die("Cannot query 966 ".mysql_error());
			
			//query with expiration date
			//$q_philhealth = mysql_query("SELECT DISTINCT(a.patient_id) FROM m_patient_philhealth a, m_family_address b, m_family_members c,m_lib_barangay d, m_patient e WHERE a.expiry_date BETWEEN '$sdate' AND '$edate' AND a.patient_id=c.patient_id AND c.family_id=b.family_id AND b.barangay_id=d.barangay_id AND a.patient_id=e.patient_id AND b.barangay_id='$_POST[sel_brgy]' ORDER by d.barangay_name ASC, e.patient_lastname ASC,a.expiry_date ASC") or die("Cannot query 968 ".mysql_error());

			//query without expiration date
			$q_philhealth = mysql_query("SELECT DISTINCT(a.patient_id) FROM m_patient_philhealth a, m_family_address b, m_family_members c,m_lib_barangay d, m_patient e WHERE a.patient_id=c.patient_id AND c.family_id=b.family_id AND b.barangay_id=d.barangay_id AND a.patient_id=e.patient_id AND b.barangay_id='$_POST[sel_brgy]' ORDER by d.barangay_name ASC, e.patient_lastname ASC,a.expiry_date ASC") or die("Cannot query 968 ".mysql_error());
			...
			//query without expiration date
			$q_philhealth = mysql_query("SELECT DISTINCT(a.patient_id) FROM m_patient_philhealth a, m_family_address b, m_family_members c,m_lib_barangay d, m_patient e WHERE a.patient_id=c.patient_id AND c.family_id=b.family_id AND b.barangay_id=d.barangay_id AND a.patient_id=e.patient_id ORDER by d.barangay_name ASC, e.patient_lastname ASC,a.expiry_date ASC") or die("Cannot query 968 ".mysql_error());
	...
	function clean_db(){
			...
			foreach($arr_tables as $key=>$value){
				...
				$q_sel_id = mysql_query("SELECT patient_id FROM $value2") or die("Cannot query 12: ".mysql_error());
				...
				while(list($pxid)=mysql_fetch_array($q_sel_id)){
					...
					if(mysql_num_rows($q_pxid)==0):
						$del_pxid = mysql_query("DELETE FROM $value2 WHERE patient_id='$pxid'") or die("Cannot query 15: ".mysql_error());
