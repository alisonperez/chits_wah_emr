...
/*
	...
	UPDATE LOG:
		- Remove morbidity to search all of the diagnosis
...
{        
    ...
    $_SESSION["w"] = $w = array(68,17,14,14,14,14,14,14,14,14,14,14,14,14,14,14,15,14,15,14,18);    
	...
	$_SESSION["header"] = $label = array('DIAGNOSIS','ICD CODE','Under 1','1-4','5-9','10-14','15-19','20-24','25-29','30-34','35-39','40-44','45-49','50-54','55-59','60-64','65&above','65-69','70&above','TOTAL');

    $this->Row($label);
    
    //$_SESSION["w2"] = $w = array(60,16,8,8,8,8,8,8,8,8,8,8,8,8,8,8,8,8,8,8,8,8,8,8,8,8,8,8,8,8,8,8,12,12);
	$_SESSION["w2"] = $w = array(68,17,7,7,7,7,7,7,7,7,7,7,7,7,7,7,7,7,7,7,7,7,7,7,7,7,7,7,7,7,7,8,7,7,7,8,7,7);
...
function q_report_header(){
    ...
    elseif($_SESSION[ques]==71 || $_SESSION[ques]==158)://morbidity monthly report
...
function show_header_bhs(){    
    if($_SESSION[ques]==70 || $_SESSION[ques]==71 || $_SESSION[ques]==158):  //applies only to W-BHS and M2 reports
        $this->Cell(0,5,'NAME OF BHS/BHC - '.$this->get_brgy_header(),0,1,L);    
    endif;        
}

function show_header_rhu(){
    if($_SESSION[ques]==70 || $_SESSION[ques]==71 || $_SESSION[ques]==74 || $_SESSION[ques]==158):   //applies only to W-BHS and M2 reports
...
function show_morbidity(){
    ...
    $w = array(68,17,7,7,7,7,7,7,7,7,7,7,7,7,7,7,7,7,7,7,7,7,7,7,7,7,7,7,7,7,7,8,7,7,7,8,7,7);
    ...
    while(list($diag_id, $count, $diag_name, $pxid, $icd10_orig,$icd_count) = mysql_fetch_array($q_diagnosis)){                
        ...
        //array_push($arr_row,$diag_name,' ');
          ...
          $q_icd = mysql_query("SELECT class_id FROM m_lib_notes_dxclass WHERE class_id='$diag_id' AND icd10 LIKE '%$icd10%'") or die("Cannot query 328 ".mysql_error());
         ...
         foreach($final_arr as $key_icd2=>$value_icd2){             
             if($key_icd==$key_icd2):  
                 ...
                 if(mysql_num_rows($q_icd_lib)!=0):
                     ...
                     $q_dxclass = mysql_query("SELECT class_name from m_lib_notes_dxclass WHERE icd10 LIKE '%$key_icd2%'") or die("Cannot query 412".mysql_error());
     ...
     foreach($final_arr2 as $key_final=>$value_final){
		...
		//END
          ...
          $this->SetFont('Arial','','10');     
          $this->Row(array('No recorded diagnosis for this period'));

	  array_push($arr_consolidate,array('No recorded diagnosis for this period'));
    endif;
