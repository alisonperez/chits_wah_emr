...
class notes extends module {
	...
	$note_link = $this->switch_notes($get_vars["notes"]);
        ...
        case "Create Notes":
            ...
            if ($result = mysql_query($sql)) {
				...
				header("location: ".$_SERVER["PHP_SELF"]."?page=".$get_vars["page"]."&menu_id=".$get_vars["menu_id"]."&consult_id=".$get_vars["consult_id"]."&ptmenu=NOTES&module=notes&notes=$note_link&notes_id=$insert_id");
            }
            break;
        case "Save Complaint":
		...
		header("location: ".$_SERVER["PHP_SELF"]."?page=".$get_vars["page"]."&menu_id=".$get_vars["menu_id"]."&consult_id=".$get_vars["consult_id"]."&ptmenu=NOTES&module=notes&notes=$note_link&notes_id=".$get_vars["notes_id"]."");                                
            break;
	...
	*/
	...
	function form_tx_sickchild(){
		...
		$patient_id = healthcenter::get_patient_id($get_vars["consult_id"]); 
		$q_px = mysql_query("SELECT a.patient_dob,b.consult_date FROM m_patient a, m_consult b WHERE a.patient_id='$patient_id' AND a.patient_id=b.patient_id AND round((TO_DAYS(date_format(consult_date,'%Y-%m-%d')) - TO_DAYS(a.patient_dob))/7,2) < 2190") or die("Cannot query: 1334"); 
