...
class appointment extends module {
	...
	function show_sms_appointments($patient_id){  
		$q_appt = mysql_query("SELECT appointment_id, appt_code, date_sending,sending_status FROM m_lib_sms_appointment WHERE patient_id='$patient_id' ORDER by date_sending") or die("Cannot query 551: ".mysql_error());

		while(list($appt_id,$appt_code,$date_send,$sending_status)=mysql_fetch_array($q_appt)){
			
			echo "<tr>";
			echo "<td>".$date_send."</td>";
			echo "<td>".appointment::get_alert_type($appt_code)."</td>";
			echo "<td>".$sending_status."</td>";
			echo "<td><a href='../site/send_sms.php?pxid=$patient_id&consult_id=$_GET[consult_id]&action=update&appt_id=$appt_id' target='new'>Update</a>&nbsp;&nbsp;";
			echo "<a href='$_SERVER[PHP_SELF]?page=$_GET[page]&menu_id=$_GET[menu_id]&consult_id=$_GET[consult_id]&ptmenu=$_GET[ptmenu]&action=delete&appt_id=$appt_id'>Delete</a>";
			echo "</tr>";
		}
	}
	...
	function get_alert_type($appt_code){
		$q_get_appt = mysql_query("SELECT sub_indicator FROM m_lib_alert_indicators WHERE alert_indicator_id='$appt_code'") or die("Cannot query 983: ".mysql_error());

		list($appt_name) = mysql_fetch_array($q_get_appt);
		return $appt_name;
	}

	function delete_sms_appt(){
		if(!empty($_GET["appt_id"]) && $_GET["action"]=='delete'):

			if($_GET["resp"]!='Y'):
				echo "<font color='red'><b>Are you sure you want to delete?</b></font>&nbsp;&nbsp;&nbsp;";
				echo "<a href='$_SERVER[PHP_SELF]?page=$_GET[page]&menu_id=$_GET[menu_id]&consult_id=$_GET[consult_id]&ptmenu=$_GET[ptmenu]&action=delete&appt_id=$_GET[appt_id]&resp=Y'>Yes</a><br><br>";
			else:
				$del_appt = mysql_query("DELETE FROM m_lib_sms_appointment WHERE appointment_id='$_GET[appt_id]'") or die("Cannot query 1002: ".mysql_error());
				
				if($del_appt):
					echo "<script language='javascript'>";
					echo "window.alert('The appointment was successfully been deleted.')";
					echo "</script>";
				endif;
			endif;
