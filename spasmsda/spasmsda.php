			...
			echo "<div style='width:100%; margin:0 auto; display:inline-block;'>";
				...
				if (isset($_REQUEST['go']) && $_REQUEST['go'] == 'Submit')
				{
					...
					echo "<tr>";
						...
						$totalPatientSQL = "SELECT count(DISTINCT a.patient_id) FROM m_lib_sms_px_enroll a, m_patient b WHERE b.patient_id = a.patient_id AND date_format(last_modified, '%Y/%m/%d') BETWEEN ";
						$totalPatientROW = "count(DISTINCT a.patient_id)";
					...
					echo "<tr id='end' class='header'>";
						...
						$countBrgySQL = "SELECT count(DISTINCT a.patient_id), a.last_modified FROM m_lib_sms_px_enroll a JOIN " .
										"m_family_members b ON b.patient_id = a.patient_id JOIN " .
										"m_patient c ON c.patient_id = a.patient_id " .
										"WHERE date_format(a.last_modified, '%Y/%m/%d') BETWEEN ";
						$countBrgyResult = "count(DISTINCT a.patient_id)";
					...
					$barangaySQL2 = "SELECT count(DISTINCT a.patient_id), a.last_modified, b.patient_id, c.family_id, c.barangay_id FROM m_lib_sms_px_enroll a JOIN " .
									"m_family_members b ON b.patient_id = a.patient_id JOIN " .
									"m_family_address c ON c.family_id = b.family_id JOIN " .
									"m_patient d ON d.patient_id = a.patient_id " .
									"WHERE c.barangay_id =";
					$barangayPatient = "count(DISTINCT a.patient_id)";
					...
					echo "<tr id='end' class='header'>";
						...
						//blankTD($s, $e);
						$countBHSsql = "SELECT count(DISTINCT a.patient_id), a.last_modified, b.patient_id, c.family_id, c.barangay_id, d.barangay_id, d.bhs_id, e.bhs_id, e.bhs_name FROM m_lib_sms_px_enroll a JOIN " .
										"m_family_members b ON b.patient_id = a.patient_id JOIN " .
										"m_family_address c ON c.family_id = b.family_id JOIN " .
										...
										"WHERE date_format(a.last_modified, '%Y/%m/%d') BETWEEN ";
						$countBHSresult = "count(DISTINCT a.patient_id)";
					...
					$bhsName = "bhs_name";
					$bhsID = "bhs_id";
					$bhsSQL2 = "SELECT count(DISTINCT a.patient_id), a.last_modified, b.patient_id, c.family_id, c.barangay_id, d.barangay_id, d.bhs_id, e.bhs_id, e.bhs_name FROM m_lib_sms_px_enroll a JOIN " .
								"m_family_members b ON b.patient_id = a.patient_id JOIN " .
								"m_family_address c ON c.family_id = b.family_id JOIN " .
								"m_lib_bhs_barangay d ON d.barangay_id = c.barangay_id JOIN " .
								"m_lib_bhs e ON e.bhs_id = d.bhs_id JOIN " .
								"m_patient f ON f.patient_id = a.patient_id " .
								"WHERE e.bhs_id =";
					$bhsPatient = "count(DISTINCT a.patient_id)";
