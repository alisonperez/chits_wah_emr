...
if(!isset($_SESSION["userid"])){
	...
	class mysqlQuery extends mysqlConnect{
		...
		public function displayQuery($maleSql,$femaleSql,$pregSql,$totalSql,$otherMaleSql,$otherFemaleSql,$i,$indicator){
			//unset($foo); $foo = array();
			switch($i){
				case 8:
				...
				default:
					...
					array_push($this->value_array,$maleValue,$maleValue);
					break;
			}
		}
		...
		public function displayTotal($Query, $service){
			if(isset($_GET['startMonth']) && isset($_GET['endMonth']) && isset($_GET['reportYear'])){
				...
				for($i=0;$i<=11;$i++){
					...
					$this->displayQuery($maleSql,$femaleSql,$pregSql,$totalSql,$otherMaleSql,$otherFemaleSql,$i,$indicator);
				}
			}
		}	
	}
