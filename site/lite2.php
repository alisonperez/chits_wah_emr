				...
				lite_connect($_POST['sync_user']);
					...
					*/
					...
					if($new_entry[4]=='m_patient_fp_pe' || $new_entry[4]=='m_patient_fp_pelvic'){
						...
						$get_mc_lite_date = mysql_query("SELECT * FROM $new_entry[4] WHERE patient_id = '$new_entry[0]' AND $where_field = '$new_entry[2]'")or die("Error 115 : ".mysql_error());
					}
		...
		if($update_array!=NULL || $update_array!=''){
			...
			foreach($update_array as $up_array => $update_value){
				...
				if($update_value[4]=='m_consult_ccdev_services' || $update_value[4]=='m_consult_vaccine' || $update_value[4]=='m_consult_ccdev_vaccine'){
					...
					$get_mc_lite_date = mysql_query("SELECT * FROM $update_value[4] WHERE patient_id = '$update_value[0]' AND $where_field = '$update_value[2]' ")or die("Error 545 : ".mysql_error());
					...
					while($get_mc_lite_result = mysql_fetch_array($get_mc_lite_date)){
						...
						if($update_value[4]=="m_consult_vaccine"){
							...
							//echo $update_value[4];
							echo "SELECT * FROM $update_value[4] WHERE mc_id = '$mc_id' AND consult_id = '$consult_id' AND patient_id = '$patient_id' AND $field_to_find = '$get_mc_lite_result[$field_to_find]'";
							$check_mc_date = mysql_query("SELECT * FROM $update_value[4] WHERE mc_id = '$mc_id' AND consult_id = '$consult_id' AND patient_id = '$patient_id' AND $field_to_find = '$get_mc_lite_result[$field_to_find]'")or die("Error 626 : ".mysql_error());
						}
						...
						if(mysql_num_rows($check_mc_date)>=1){
							...
							for($i=0;$i<$field_count;$i++){
								...
								if($field_name=='vaccine_id' || $field_name=='mc_id' || $field_name=='consult_id' || $field_name=='ccdev_id' || $field_name=='patient_id' || $field_name==$field_to_find){
									$value = $$field_name;
	...
	function update_m_push(){
		...
		foreach($list as $key => $value){
			switch ($type){
				...
				case 'update':
					if($vaue[4]){
						$update_sql = "UPDATE m_push_status SET push_flag='N' WHERE lite_patient_id = '$value[0]' AND emr_patient_id = '$value[1]' AND table_name='$value[4]' AND entry_date='$value[5]'";
					}else{
