	...
	<body>
		<?php
			if (isset($_REQUEST['eDate']) && $_SESSION["userid"]!="")
			{
				...
				$sqlFF = "SELECT * FROM m_patient a JOIN m_lib_sms_px_enroll b ON b.patient_id = a.patient_id  WHERE DATE_FORMAT( b.last_modified,  '%Y/%m/%d' ) BETWEEN '$startDate' AND '$endDate'
						AND a.patient_id NOT IN (SELECT patient_id FROM m_family_members)";	
				$queryFF = mysqli_query($dbConnect,$sqlFF);
				
				if(mysqli_num_rows($queryFF))
				{
					echo "<table border=1 style='border-collapse:collapse'>";
						echo "<tr>";
							echo "<th colspan='7'>Patients Not Recorded in Family Folder</th>";
						echo "</tr>";
						echo "<tr>";
							echo "<th colspan='2'>Patient ID</th>";
							echo "<th>Last Name</th>";
							echo "<th>First Name</th>";
							echo "<th>Middle Name</th>";
							echo "<th>Program ID</th>";
							echo "<th>Date of Enrollment</th>";
						echo "</tr>";
						$count = 0;	
						while($resultFF = mysqli_fetch_array($queryFF))
						{
							$count += 1;
							echo "<tr>";
								echo "<td>$count.</td>";
								echo "<td  width='60px'>".$resultFF['patient_id']."</td>";
								echo "<td>".$resultFF['patient_lastname']."</td>";
								echo "<td>".$resultFF['patient_firstname']."</td>";
								echo "<td>".$resultFF['patient_middle']."</td>";
								echo "<td>".$resultFF['program_id']."</td>";
								echo "<td align='center'>".date("m/d/Y", strtotime($resultFF['last_modified']))."</td>";
							echo "</tr>";
						}
					echo "</table>";
